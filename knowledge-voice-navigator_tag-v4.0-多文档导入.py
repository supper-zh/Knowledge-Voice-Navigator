...
"""
        ...
        # 尝试加载文档库
        if self.load_document_library():
            self.status_bar.config(text=f"已加载文档库，共 {len(self.document_library)} 个文档")

        # 启动定期保存
        self.save_document_library_periodically()
    ...
    def batch_import_documents(self):
        ...
        # 启动导入进度对话框
        self.import_documents(file_paths)

    def import_documents(self, file_paths):
        """处理多个文档的导入"""
        if not file_paths:
            return
        ...
        progress_dialog.title("导入文档")
        progress_dialog.transient(self.root)
        progress_dialog.grab_set()
        progress_dialog.geometry("400x300")

        # 显示标题
        tk.Label(progress_dialog, text="正在导入文档...", font=("Arial", 12, "bold")).pack(pady=10)

        # 显示当前处理的文件
        current_file_var = tk.StringVar(value="准备中...")
        tk.Label(progress_dialog, textvariable=current_file_var).pack(pady=5)

        # 添加进度条
        progress_var = tk.DoubleVar()
        progress_bar = ttk.Progressbar(progress_dialog, variable=progress_var, maximum=len(file_paths))
        progress_bar.pack(fill=tk.X, padx=20, pady=10)

        # 添加状态文本
        status_var = tk.StringVar(value="0/{} 文件已处理".format(len(file_paths)))
        tk.Label(progress_dialog, textvariable=status_var).pack(pady=5)

        # 添加日志文本框
        log_text = scrolledtext.ScrolledText(progress_dialog, height=8, width=50)
        ...
        def process_import():
            # 初始化文档库
            if not hasattr(self, 'document_library'):
                self.document_library = {}  # 存储所有导入的文档
                self.document_index = {}  # 关键词到文档的映射
                self.document_relations = {}  # 文档间的关联关系
            ...
            for i, file_path in enumerate(file_paths):
                try:
                    ...
                    current_file_var.set(f"正在处理: {filename}")
                    add_log(f"开始导入: {filename}")
                    ...
                    try:
                        ...
                        try:
                            ...
                            add_log(f"读取文件失败: {str(e)}")
                            errors.append((file_path, str(e)))
                            continue

                    # 解析文档结构
                    doc_info = self.parse_document(file_path, content)

                    # 添加到文档库
                    self.document_library[file_path] = doc_info

                    # 更新索引
                    self.update_document_index(file_path, doc_info)

                    # 更新计数和进度
                    imported_count += 1
                    progress_var.set(i + 1)
                    status_var.set(f"{i + 1}/{len(file_paths)} 文件已处理")
                    add_log(f"成功导入: {filename}")

                    # 强制更新界面
                    progress_dialog.update()

                except Exception as e:
                    add_log(f"处理文件 {os.path.basename(file_path)} 出错: {str(e)}")
                    errors.append((file_path, str(e)))
            ...
            # 完成导入
            add_log(f"导入完成。成功: {imported_count}, 失败: {len(errors)}")
            status_var.set(f"完成: {imported_count}/{len(file_paths)} 文件已导入")

            # 更新界面状态
            self.update_document_browser()
            ...
            if errors:
                ...
                self.messagebox.showwarning("导入错误",
                                            f"导入过程中发生 {len(errors)} 个错误:\n\n{error_log}")
        ...
        # 添加关闭按钮
        close_button = tk.Button(progress_dialog, text="关闭", width=10, state=tk.DISABLED,
    ...
    def parse_document(self, file_path, content):
        """解析单个文档，提取结构和关键信息"""
        # 初始化文档信息
        doc_info = {
            'path': file_path,
            'filename': os.path.basename(file_path),
            'title': os.path.basename(file_path),  # 默认使用文件名作为标题
            'headings': [],
            'content': content,
            'keywords': [],
            'summary': "",
        ...
        if doc_info['is_markdown']:
            ...
            for match in heading_pattern.finditer(content):
                ...
                # 如果是一级标题且文档标题还是默认值，则更新文档标题
                if level == 1 and doc_info['title'] == os.path.basename(file_path):
                    doc_info['title'] = clean_heading
        else:
            # 文本文件标题识别
            # (使用和原有代码相同的文本文件标题识别逻辑)
            # ...

            # 按位置排序
            headings.sort(key=lambda x: x['position'])
            doc_info['headings'] = headings

            # 提取关键词
            try:
                doc_info['keywords'] = self.extract_document_keywords(content)
            except Exception as e:
                print(f"提取关键词失败: {str(e)}")

            # 生成简要摘要
            try:
                doc_info['summary'] = self.generate_document_summary(content, headings)
            except Exception as e:
                print(f"生成摘要失败: {str(e)}")

        return doc_info

    def extract_document_keywords(self, content, max_keywords=20):
        """从文档内容中提取关键词"""
        # 使用已有的extract_keywords方法，但针对整个文档做一些优化
    ...
    def generate_document_summary(self, content, headings, max_length=200):
        """生成文档摘要"""
        # 简单实现：取第一段非空文本
        ...
        for para in paragraphs:
            para = para.strip()
            if para and not re.match(r'^#+\s+', para):
                # 截断到最大长度
    ...
    def analyze_document_relations(self):
        """分析文档间的关联关系"""
        # 初始化关系图
        self.document_relations = {}
        for path in self.document_library:
            self.document_relations[path] = []

        # 根据关键词重叠度计算文档关联
        for doc1_path in self.document_library:
            doc1 = self.document_library[doc1_path]
            doc1_keywords = set(doc1['keywords'])

            for doc2_path in self.document_library:
                if doc1_path == doc2_path:
                    continue  # 跳过自身

                doc2 = self.document_library[doc2_path]
                doc2_keywords = set(doc2['keywords'])

                # 计算关键词重叠度
                overlap = len(doc1_keywords.intersection(doc2_keywords))

                # 如果有足够的重叠，建立关联
                if overlap >= 3:  # 至少3个共同关键词
                    relation = {
                        'path': doc2_path,
                        'title': doc2['title'],
                        'overlap': overlap,
                        'common_keywords': list(doc1_keywords.intersection(doc2_keywords))
                    }
                    self.document_relations[doc1_path].append(relation)

        # 对每个文档的关联按重叠度排序
        for doc_path in self.document_relations:
            self.document_relations[doc_path].sort(key=lambda x: x['overlap'], reverse=True)

    def update_document_browser(self):
        """更新多文档浏览界面"""
        # 如果还没有文档库窗口，创建一个
        if not hasattr(self, 'document_browser') or not self.document_browser:
            self.create_document_browser()
        else:
            # 刷新已有窗口
            self.refresh_document_browser()

    def create_document_browser(self):
        """创建多文档浏览器窗口"""
        if not hasattr(self, 'document_library') or not self.document_library:
            self.messagebox.showinfo("提示", "请先导入文档")
            return

        # 创建新窗口
        self.document_browser = tk.Toplevel(self.root)
        self.document_browser.title("多文档浏览器")
        self.document_browser.geometry("900x600")

        # 创建分割窗口
        browser_paned = tk.PanedWindow(self.document_browser, orient=tk.HORIZONTAL)
        browser_paned.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)

        # 左侧：文档列表
        left_frame = tk.Frame(browser_paned, width=300)
        ...
        tk.Label(search_frame, text="搜索文档:").pack(side=tk.LEFT, padx=5)
        doc_search_var = tk.StringVar()
        doc_search_entry = tk.Entry(search_frame, textvariable=doc_search_var)
        doc_search_entry.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=5)
        doc_search_button = tk.Button(search_frame, text="搜索",
                                      command=lambda: self.search_documents(doc_search_var.get()))
        doc_search_button.pack(side=tk.LEFT, padx=5)

        # 文档列表
        list_frame = tk.Frame(left_frame)
        list_frame.pack(fill=tk.BOTH, expand=True, pady=5)

        docs_listbox = tk.Listbox(list_frame, selectmode=tk.SINGLE)
        docs_scrollbar = tk.Scrollbar(list_frame, orient=tk.VERTICAL, command=docs_listbox.yview)
        docs_listbox.config(yscrollcommand=docs_scrollbar.set)
        docs_scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
        docs_listbox.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)

        # 填充文档列表
        for doc_path in self.document_library:
            doc_info = self.document_library[doc_path]
            docs_listbox.insert(tk.END, doc_info['title'])

        # 右侧：文档信息
        right_frame = tk.Frame(browser_paned)

        # 文档信息区
        info_frame = tk.Frame(right_frame)
        info_frame.pack(fill=tk.X, pady=5)

        doc_title_var = tk.StringVar(value="选择一个文档")
        title_label = tk.Label(info_frame, textvariable=doc_title_var, font=("Arial", 14, "bold"))
        title_label.pack(anchor=tk.W, pady=5)

        doc_path_var = tk.StringVar()
        path_label = tk.Label(info_frame, textvariable=doc_path_var, font=("Arial", 9))
        ...
        open_button = tk.Button(button_frame, text="打开文档", width=12,
                                command=lambda: self.open_selected_document(docs_listbox))
        open_button.pack(side=tk.LEFT, padx=5)

        remove_button = tk.Button(button_frame, text="从库中移除", width=12,
                                  command=lambda: self.remove_document(docs_listbox))
        remove_button.pack(side=tk.LEFT, padx=5)

        # 文档摘要和信息
        summary_frame = tk.LabelFrame(right_frame, text="文档摘要")
        summary_frame.pack(fill=tk.X, pady=5, ipady=5)
        ...
        # 关键词区域
        keywords_frame = tk.LabelFrame(right_frame, text="关键词")
        keywords_frame.pack(fill=tk.X, pady=5, ipady=5)

        keywords_text = tk.Text(keywords_frame, height=3, wrap=tk.WORD)
        keywords_text.pack(fill=tk.X, padx=5, pady=5)
        keywords_text.config(state=tk.DISABLED)

        # 关联文档区域
        relations_frame = tk.LabelFrame(right_frame, text="相关文档")
        relations_frame.pack(fill=tk.BOTH, expand=True, pady=5)

        relations_list = tk.Listbox(relations_frame)
        relations_scroll = tk.Scrollbar(relations_frame, orient=tk.VERTICAL, command=relations_list.yview)
        relations_list.config(yscrollcommand=relations_scroll.set)
        relations_scroll.pack(side=tk.RIGHT, fill=tk.Y)
        relations_list.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=5, pady=5)

        # 添加到分割窗口
        browser_paned.add(left_frame)
        browser_paned.add(right_frame)

        # 设置初始比例
        browser_paned.paneconfigure(left_frame, minsize=200)
        browser_paned.paneconfigure(right_frame, minsize=400)
        ...
        def on_doc_select(event):
            ...
            # 获取选中的文档
            index = selection[0]
            doc_path = list(self.document_library.keys())[index]
            doc_info = self.document_library[doc_path]

            # 更新文档信息显示
            doc_title_var.set(doc_info['title'])
            doc_path_var.set(doc_path)

            # 更新摘要
            summary_text.config(state=tk.NORMAL)
            summary_text.delete(1.0, tk.END)
            summary_text.insert(tk.END, doc_info['summary'])
            summary_text.config(state=tk.DISABLED)

            # 更新关键词
            keywords_text.config(state=tk.NORMAL)
            keywords_text.delete(1.0, tk.END)
            keywords_text.insert(tk.END, ", ".join(doc_info['keywords'][:20]))
            keywords_text.config(state=tk.DISABLED)

            # 更新关联文档
            relations_list.delete(0, tk.END)
            if doc_path in self.document_relations:
                for relation in self.document_relations[doc_path]:
                    relations_list.insert(tk.END, f"{relation['title']} [{relation['overlap']}]")

        docs_listbox.bind('<<ListboxSelect>>', on_doc_select)

        # 绑定关联文档双击事件
        def on_relation_double_click(event):
            selection = relations_list.curselection()
            if not selection:
                return

            # 获取当前选中文档
            doc_selection = docs_listbox.curselection()
            if not doc_selection:
                return

            doc_index = doc_selection[0]
            doc_path = list(self.document_library.keys())[doc_index]

            # 获取选中的关联文档
            rel_index = selection[0]
            if doc_path in self.document_relations and rel_index < len(self.document_relations[doc_path]):
                rel_doc_path = self.document_relations[doc_path][rel_index]['path']

                # 在文档列表中查找并选中相关文档
                for i, path in enumerate(self.document_library.keys()):
                    if path == rel_doc_path:
                        docs_listbox.selection_clear(0, tk.END)
                        docs_listbox.selection_set(i)
                        docs_listbox.see(i)
                        docs_listbox.event_generate('<<ListboxSelect>>')
                        break

        relations_list.bind('<Double-1>', on_relation_double_click)

        # 保存引用
        self.docs_listbox = docs_listbox
        self.doc_search_var = doc_search_var
    ...
    def search_documents(self, query):
        """搜索文档库"""
        if not query or not hasattr(self, 'document_library') or not self.document_library:
            return
        ...
        for doc_path in self.document_library:
            ...
            for keyword in keywords:
                if keyword.lower() in doc_info['title'].lower():
                    match_score += 3  # 标题匹配权重高
                    matched_keywords.append(keyword)

            # 检查关键词匹配
            for keyword in keywords:
                if keyword in doc_info['keywords']:
                    match_score += 1
            ...
            # 如果有匹配，添加到结果
            if match_score > 0:
                matches.append((doc_path, match_score, matched_keywords))

        # 按匹配得分排序
        matches.sort(key=lambda x: x[1], reverse=True)

        # 更新列表
        for doc_path, score, _ in matches:
            doc_info = self.document_library[doc_path]
            self.docs_listbox.insert(tk.END, f"{doc_info['title']} [{score}]")

        # 提示搜索结果
        if matches:
            self.document_browser.title(f"多文档浏览器 - 搜索 \"{query}\" ({len(matches)} 个结果)")
        else:
            self.document_browser.title(f"多文档浏览器 - 搜索 \"{query}\" (无结果)")
            self.docs_listbox.insert(tk.END, "没有找到匹配的文档")

    def open_selected_document(self, listbox):
        """打开选中的文档"""
        ...
        try:
            ...
            # 打开文档
            self.knowledge_path = doc_path
            try:
                with open(doc_path, 'r', encoding='utf-8') as file:
                    self.knowledge_base = file.read()
            except UnicodeDecodeError:
                with open(doc_path, 'r', encoding='gbk') as file:
                    self.knowledge_base = file.read()
            ...
            self.status_bar.config(text=f"已加载文件: {doc_path}")
            ...
            self.messagebox.showerror("错误", f"打开文档失败: {str(e)}")

    def remove_document(self, listbox):
        """从文档库中移除选中的文档"""
        selection = listbox.curselection()
        if not selection:
            return
        ...
        try:
            ...
            # 确认删除
            if not messagebox.askyesno("确认", f"确定要从文档库中移除 '{doc_info['title']}' 吗?"):
                return

            # 从文档库中移除
            del self.document_library[doc_path]

            # 从索引中移除
            for keyword in self.document_index:
                if doc_path in self.document_index[keyword]:
                    self.document_index[keyword].remove(doc_path)

            # 从关系图中移除
            if doc_path in self.document_relations:
                del self.document_relations[doc_path]
            for other_doc in self.document_relations:
                self.document_relations[other_doc] = [r for r in self.document_relations[other_doc]
                                                      if r['path'] != doc_path]
            ...
            self.messagebox.showerror("错误", f"移除文档失败: {str(e)}")

    def refresh_document_browser(self):
        """刷新文档浏览器界面"""
        if not hasattr(self, 'document_browser') or not self.document_browser:
            return

        # 更新标题
        self.document_browser.title(f"多文档浏览器 ({len(self.document_library)} 个文档)")

        # 更新文档列表
        if hasattr(self, 'docs_listbox'):
            self.docs_listbox.delete(0, tk.END)
            for doc_path in self.document_library:
                doc_info = self.document_library[doc_path]
                self.docs_listbox.insert(tk.END, doc_info['title'])

    def save_document_library(self):
        """保存文档库到本地文件"""
        library_file = "document_library.dat"
        ...
        try:
            ...
            for doc_path in self.document_library:
                doc_info = self.document_library[doc_path]
                # 只保存必要信息
                simplified_library[doc_path] = {
                    'title': doc_info['title'],
                    'keywords': doc_info['keywords'],
                    'summary': doc_info['summary'],
                    'last_modified': doc_info['last_modified'],
                    'is_markdown': doc_info['is_markdown']
                }
            ...
            with open(library_file, 'w', encoding='utf-8') as f:
                import json
                json.dump({
                    'library': simplified_library,
                    'relations': self.document_relations
                }, f, ensure_ascii=False, indent=2)

            return True
        except Exception as e:
            print(f"保存文档库失败: {str(e)}")
            return False
    ...
    def load_document_library(self):
        ...
        try:
            with open(library_file, 'r', encoding='utf-8') as f:
                ...
                # 加载文档信息
                for doc_path, saved_info in data.get('library', {}).items():
                    # 验证文件是否存在
                    ...
                    try:
                        ...
                        try:
                            with open(doc_path, 'r', encoding='gbk') as doc_file:
                                content = doc_file.read()
                        except Exception:
                            continue  # 跳过无法读取的文件

                    # 解析文档结构
                    doc_info = self.parse_document(doc_path, content)

                    # 合并保存的信息
                    for key in saved_info:
                        doc_info[key] = saved_info[key]

                    # 添加到文档库
                    self.document_library[doc_path] = doc_info

                    # 更新索引
                    self.update_document_index(doc_path, doc_info)

                # 加载关系图
                self.document_relations = data.get('relations', {})

                return True
        except Exception as e:
            print(f"加载文档库失败: {str(e)}")
            return False

    def save_document_library_periodically(self):
        """定期自动保存文档库"""
        if hasattr(self, 'document_library') and self.document_library:
            self.save_document_library()

        # 每5分钟保存一次
        self.root.after(300000, self.save_document_library_periodically)


    def update_document_index(self, file_path, doc_info):
        """更新文档索引"""
        # 为每个关键词创建或更新索引项
        for keyword in doc_info['keywords']:
            if keyword not in self.document_index:
                self.document_index[keyword] = []
            if file_path not in self.document_index[keyword]:
                self.document_index[keyword].append(file_path)

        # 为每个标题创建索引
        for heading in doc_info['headings']:
            heading_text = heading['text'].lower()
            heading_keywords = self.extract_keywords(heading_text)
    ...
    def import_directory(self):
        """导入整个目录的文档"""
        # 选择目录
        directory = filedialog.askdirectory(title="选择知识库文件夹")
        ...
        if not file_paths:
            self.messagebox.showinfo("提示", "所选文件夹中没有找到支持的文档文件(.md, .txt)")
            return
    ...
    def create_ui(self):
        ...
        # 文件菜单中添加批量导入选项
        self.file_menu.add_command(label="批量导入文档", command=self.batch_import_documents)
        self.file_menu.add_command(label="导入文件夹", command=self.import_directory)
        ...
        self.menu_bar.add_cascade(label="文档库", menu=self.library_menu)

        self.library_menu.add_command(label="浏览文档库", command=self.show_document_browser)
        self.library_menu.add_command(label="搜索文档库", command=self.search_document_library)
        self.library_menu.add_separator()
        self.library_menu.add_command(label="导出文档库", command=self.export_document_library)
        self.library_menu.add_command(label="导入文档库", command=self.import_document_library)
        self.library_menu.add_separator()
        self.library_menu.add_command(label="清空文档库", command=self.clear_document_library)
        # 帮助菜单
    ...
    def show_document_browser(self):
        """显示文档浏览器"""
        if not hasattr(self, 'document_library') or not self.document_library:
            self.messagebox.showinfo("提示", "文档库为空。请先导入文档。")
            return

        self.update_document_browser()

    def search_document_library(self):
        """搜索文档库对话框"""
        if not hasattr(self, 'document_library') or not self.document_library:
            self.messagebox.showinfo("提示", "文档库为空。请先导入文档。")
            return
        ...
        search_dialog.title("搜索文档库")
        search_dialog.geometry("400x150")
        search_dialog.transient(self.root)
        search_dialog.grab_set()

        # 创建搜索框
        tk.Label(search_dialog, text="输入搜索关键词:").pack(pady=(15, 5))

        search_var = tk.StringVar()
        search_entry = tk.Entry(search_dialog, textvariable=search_var, width=40)
        ...
        # 添加按钮
        search_button = tk.Button(button_frame, text="搜索", width=10, command=do_search)
        search_button.pack(side=tk.LEFT, padx=5)

        cancel_button = tk.Button(button_frame, text="取消", width=10, command=search_dialog.destroy)
        cancel_button.pack(side=tk.LEFT, padx=5)

        # 绑定回车键
        search_entry.bind("<Return>", lambda e: do_search())

    def export_document_library(self):
        """导出文档库到文件"""
        if not hasattr(self, 'document_library') or not self.document_library:
            self.messagebox.showinfo("提示", "文档库为空。没有内容可导出。")
            return

        # 选择导出文件
        file_path = filedialog.asksaveasfilename(
            title="导出文档库",
            defaultextension=".kdl",
            filetypes=[("知识库文档库", "*.kdl"), ("所有文件", "*.*")]
        )
        ...
        try:
            ...
            for doc_path in self.document_library:
                doc_info = self.document_library[doc_path]
                # 相对路径转换，这样导入到其他系统时更容易匹配
                rel_path = os.path.basename(doc_path)

                export_data['library'][rel_path] = {
                    'title': doc_info['title'],
                    'keywords': doc_info['keywords'],
                    'summary': doc_info['summary'],
                    'is_markdown': doc_info['is_markdown'],
            ...
            self.messagebox.showinfo("成功", f"已成功导出文档库，包含 {len(self.document_library)} 个文档")

        except Exception as e:
            self.messagebox.showerror("错误", f"导出文档库失败: {str(e)}")

    def import_document_library(self):
        """从文件导入文档库"""
        # 选择导入文件
        ...
        try:
            ...
            # 询问导入方式
            merge = messagebox.askyesno("导入选项","是否合并到当前文档库？\n选择\"是\"合并，选择\"否\"替换当前文档库。")
            if not merge:
                # 清空当前库
                self.document_library = {}
                self.document_index = {}
            ...
            for rel_path, import_info in import_data.get('library', {}).items():
                original_path = import_info.get('original_path', '')
                ...
                if os.path.exists(original_path):
                    ...
                    # 尝试在当前目录寻找
                    local_path = os.path.join(os.path.dirname(file_path), rel_path)
                    if os.path.exists(local_path):
                        doc_path = local_path
                    else:
                        # 尝试让用户选择文件
                        self.messagebox.showinfo("找不到文件",
                                                 f"找不到文档: {rel_path}\n请手动选择此文件。")
                        doc_path = filedialog.askopenfilename(
                            title=f"选择文档 {rel_path}",
                            initialfile=rel_path
                        )
                ...
                try:
                    ...
                    # 合并导入的信息
                    for key in import_info:
                        if key != 'original_path':
                            doc_info[key] = import_info[key]
                    ...
                    error_count += 1
                    print(f"导入文档 {doc_path} 失败: {str(e)}")

            # 更新关系
            if 'relations' in import_data:
                for doc_path, relations in import_data['relations'].items():
                    # 查找原始路径对应的新路径
                    new_path = None
                    for path in self.document_library:
                        if os.path.basename(path) == os.path.basename(doc_path):
                            new_path = path
                            break

                    if new_path:
                        self.document_relations[new_path] = []

                        # 更新关系中的路径
                        for relation in relations:
                            rel_doc_path = relation['path']
                            new_rel_path = None

                            # 查找关联的文档
                            for path in self.document_library:
                                if os.path.basename(path) == os.path.basename(rel_doc_path):
                                    new_rel_path = path
                                    break

                            if new_rel_path:
                                # 更新路径引用
                                relation['path'] = new_rel_path
                                self.document_relations[new_path].append(relation)

            # 保存导入的库
            self.save_document_library()

            # 显示结果
            self.messagebox.showinfo("导入完成",
                                     f"导入完成。成功: {import_count}, 失败: {error_count}")

            # 刷新浏览器
            if hasattr(self, 'document_browser') and self.document_browser:
                self.refresh_document_browser()

        except Exception as e:
            self.messagebox.showerror("错误", f"导入文档库失败: {str(e)}")

    def clear_document_library(self):
        """清空文档库"""
        if not hasattr(self, 'document_library') or not self.document_library:
            self.messagebox.showinfo("提示", "文档库已经为空。")
            return

        # 确认清空
        if not messagebox.askyesno("确认",
                                   f"确定要清空文档库吗？这将移除所有 {len(self.document_library)} 个文档的索引和关联。\n\n注意：不会删除原始文件。"):
            return
        ...
        # 刷新浏览器
        if hasattr(self, 'document_browser') and self.document_browser:
            self.document_browser.destroy()
            self.document_browser = None

        self.messagebox.showinfo("操作完成", "文档库已清空。")
