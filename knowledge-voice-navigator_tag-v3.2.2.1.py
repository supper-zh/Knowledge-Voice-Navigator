...
"""
    ...
    def create_ui(self):
        ...
        # self.main_paned = tk.PanedWindow(self.root, orient=tk.HORIZONTAL, sashwidth=10)
        self.main_paned = tk.PanedWindow(self.root, orient=tk.HORIZONTAL,
                                         sashwidth=10,  # 更宽的分隔条
    ...
    def show_sort_options(self):
        ...
        sort_dialog.title("排序选项")
        sort_dialog.transient(self.root)
        sort_dialog.grab_set()
        sort_dialog.geometry("300x180")

        tk.Label(sort_dialog, text="选择排序方式:", anchor="w").pack(fill=tk.X, padx=20, pady=10)
        ...
        # 按钮框架
        button_frame = tk.Frame(sort_dialog)
        ...
        tk.Button(button_frame, text="应用", width=10, command=apply_sort).pack(side=tk.RIGHT, padx=10)
        tk.Button(button_frame, text="取消", width=10, command=sort_dialog.destroy).pack(side=tk.RIGHT, padx=5)
    ...
    def show_filter_options(self):
        ...
        filter_dialog.title("过滤选项")
        filter_dialog.transient(self.root)
        filter_dialog.grab_set()
        filter_dialog.geometry("320x280")

        # 匹配类型过滤
        type_frame = tk.LabelFrame(filter_dialog, text="匹配类型", padx=10, pady=5)
        ...
        # 相关度过滤
        relevance_frame = tk.LabelFrame(filter_dialog, text="最小相关度", padx=10, pady=5)
        relevance_frame.pack(fill=tk.X, padx=10, pady=5)

        relevance_var = tk.IntVar(value=self.search_filter['min_relevance'])
        relevance_label = tk.Label(relevance_frame, text=f"最小相关度: {relevance_var.get()}%")
        relevance_label.pack(anchor="w", pady=2)

        def update_relevance_label(val):
            relevance_label.config(text=f"最小相关度: {int(float(val))}%")

        relevance_scale = tk.Scale(
            relevance_frame,
            from_=0,
            to=100,
            orient=tk.HORIZONTAL,
            variable=relevance_var,
            command=update_relevance_label
        )
        relevance_scale.pack(fill=tk.X, pady=2)

        # 章节过滤
        chapter_frame = tk.LabelFrame(filter_dialog, text="按章节过滤", padx=10, pady=5)
        chapter_frame.pack(fill=tk.X, padx=10, pady=5)

        # 获取所有一级章节
        chapters = ["全部章节"]
        if hasattr(self, 'heading_positions') and self.heading_positions:
            level1_headings = [h['text'] for h in self.heading_positions if h.get('level', 0) == 1]
            if level1_headings:
                chapters.extend(level1_headings)
        ...
        if len(chapters) > 8:
            ...
            chapter_combo.pack(fill=tk.X, pady=5)
        else:
            # 否则使用单选按钮
        ...
        tk.Button(button_frame, text="应用", width=10, command=apply_filter).pack(side=tk.RIGHT, padx=10)
        tk.Button(button_frame, text="取消", width=10, command=filter_dialog.destroy).pack(side=tk.RIGHT, padx=5)
