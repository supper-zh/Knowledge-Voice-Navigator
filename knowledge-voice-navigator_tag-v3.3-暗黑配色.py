...
"""
    ...
    def create_ui(self):
        ...
        self.view_menu.add_separator()
        self.view_menu.add_radiobutton(label="亮色主题",
                                       variable=self.theme_var,
                                       value="light",
                                       command=self.change_theme)
        self.view_menu.add_radiobutton(label="暗色主题",
                                       variable=self.theme_var,
                                       value="dark",
                                       command=self.change_theme)
    ...
    def change_theme(self):
        ...
        # 保存用户偏好
        self.save_theme_preference(theme_value)
    ...
    def apply_theme(self):
        ...
        # 配置所有框架
        for frame in [self.control_frame, self.tag_frame, self.match_frame,
                      self.content_frame, self.toc_frame]:
            frame.configure(bg=self.current_theme["bg"])

        # 配置所有标签
        for label in [self.status_label, self.match_label, self.toc_label]:
            label.configure(bg=self.current_theme["bg"],
                            fg=self.current_theme["fg"])

        # 配置状态栏
        self.status_bar.configure(bg=self.current_theme["status_bg"],
                                  fg=self.current_theme["fg"])

        # 配置按钮 - 监听按钮保持原有的绿色/红色
        self.search_button.configure(bg=self.current_theme["button_bg"],
                                     fg=self.current_theme["button_fg"])
        self.clear_button.configure(bg=self.current_theme["button_bg"],
                                    fg=self.current_theme["button_fg"])

        # 配置内容文本区域 - 这是最关键的部分
        self.content_text.configure(bg=self.current_theme["bg"],
                                    fg=self.current_theme["fg"],
                                    insertbackground=self.current_theme["fg"],  # 光标颜色
                                    selectbackground=self.current_theme["selection_bg"],
        ...
        # 配置匹配列表
        self.match_list.configure(bg=self.current_theme["bg"],
                                  fg=self.current_theme["fg"],
                                  selectbackground=self.current_theme["selection_bg"],
                                  selectforeground=self.current_theme["highlight_fg"])
        ...
        if self.knowledge_base:
            self.content_text.tag_config("search_highlight",
                                         background=self.current_theme["search_highlight"],
                                         foreground=self.current_theme["highlight_fg"])

        # 保存主题设置
        self.save_theme_preference(self.theme_var.get())

    def update_text_tags(self):
        """更新文本区域的所有标签样式以匹配当前主题"""
        # 更新标题样式
        self.content_text.tag_configure("h1",
                                        font=("Arial", 18, "bold"),
                                        foreground=self.current_theme["heading1"])
        self.content_text.tag_configure("h2",
                                        font=("Arial", 16, "bold"),
                                        foreground=self.current_theme["heading2"])
        self.content_text.tag_configure("h3",
                                        font=("Arial", 14, "bold"),
                                        foreground=self.current_theme["heading3"])
        self.content_text.tag_configure("h4",
                                        font=("Arial", 12, "bold"),
                                        foreground=self.current_theme["fg"])

        # 更新其他标签样式
        self.content_text.tag_configure("bold",
                                        font=("Courier New", self.current_font_size, "bold"),
                                        foreground=self.current_theme["fg"])
        self.content_text.tag_configure("italic",
                                        font=("Courier New", self.current_font_size, "italic"),
                                        foreground=self.current_theme["fg"])
        self.content_text.tag_configure("code",
                                        font=("Consolas", self.current_font_size),
                                        background=self.current_theme["code_bg"],
                                        foreground=self.current_theme["fg"])
        ...
        self.content_text.tag_configure("link",
                                        foreground=self.current_theme["link"],
                                        underline=1)

        # 高亮位置标签
        self.content_text.tag_configure("position_highlight",
                                        background=self.current_theme["highlight_bg"],
                                        foreground=self.current_theme["highlight_fg"])
    ...
    def configure_treeview(self):
        ...
        style.configure("Treeview",
                        background=self.current_theme["toc_bg"],
                        foreground=self.current_theme["fg"],
                        fieldbackground=self.current_theme["toc_bg"])
        ...
        # 配置选中项样式
        style.map('Treeview',
                  background=[('selected', self.current_theme["selection_bg"])],
    ...
    def update_tag_buttons(self):
        ...
        for tag_info in self.tag_buttons:
            ...
            # 在暗色模式中，使用更暗的标签色调，但保持标签的唯一颜色
            if self.theme_var.get() == "dark":
                # 使用标签文本生成颜色，但调暗
                color_seed = hash(tag_text) % 1000
                r = min(130, max(80, (color_seed % 5) * 10 + 80))
                g = min(140, max(100, ((color_seed // 5) % 5) * 10 + 100))
                b = min(150, max(120, ((color_seed // 25) % 5) * 10 + 120))
            ...
            # 更新组件颜色
            tag_container.config(bg=bg_color)
            tag_button.config(bg=bg_color, fg=self.current_theme["button_fg"])
            count_label.config(bg=bg_color, fg=self.current_theme["button_fg"])
            close_button.config(bg=bg_color, fg=self.current_theme["button_fg"])

    def save_theme_preference(self, theme_value):
        """保存用户主题偏好"""
        # 使用与标签相同的基本路径策略
        if self.knowledge_path:
            base_name = os.path.splitext(self.knowledge_path)[0]
            settings_file = f"{base_name}.settings"
        else:
        ...
        try:
            ...
            self.status_bar.config(text=f"保存设置失败: {str(e)}")
    ...
    def define_color_schemes(self):
        ...
        self.light_theme = {
            "bg": "#FFFFFF",  # 背景色
            "fg": "#333333",  # 前景色（文本）
            "highlight_bg": "#E0E0E0",  # 高亮背景
            "highlight_fg": "#000000",  # 高亮文本
            "button_bg": "#F0F0F0",  # 按钮背景
            "button_fg": "#333333",  # 按钮文本
            "search_highlight": "#FFFF66",  # 搜索高亮
            "link": "#0000FF",  # 链接颜色
            "heading1": "#0066CC",  # 一级标题
            "heading2": "#0099CC",  # 二级标题
            "heading3": "#33CCCC",  # 三级标题
            "code_bg": "#F8F8F8",  # 代码背景
            "selection_bg": "#B0C4DE",  # 选择背景
            "tag_bg": "#E6E6E6",  # 标签背景
            "status_bg": "#F0F0F0",  # 状态栏背景
            "toc_bg": "#FFFFFF",  # 目录背景
            "scrollbar": "#C0C0C0"  # 滚动条
        }
        ...
        self.dark_theme = {
            "bg": "#282C34",  # 背景色
            "fg": "#ABB2BF",  # 前景色（文本）
            "highlight_bg": "#3E4451",  # 高亮背景
            "highlight_fg": "#FFFFFF",  # 高亮文本
            "button_bg": "#3B3F4B",  # 按钮背景
            "button_fg": "#FFFFFF",  # 按钮文本
            "search_highlight": "#665C00",  # 搜索高亮
            "link": "#61AFEF",  # 链接颜色
            "heading1": "#61AFEF",  # 一级标题
            "heading2": "#56B6C2",  # 二级标题
            "heading3": "#98C379",  # 三级标题
            "code_bg": "#2C323C",  # 代码背景
            "selection_bg": "#528BFF",  # 选择背景
            "tag_bg": "#3B4048",  # 标签背景
            "status_bg": "#21252B",  # 状态栏背景
            "toc_bg": "#282C34",  # 目录背景
            "scrollbar": "#4B5363"  # 滚动条
        }
