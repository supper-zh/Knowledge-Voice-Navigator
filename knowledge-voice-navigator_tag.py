...
class KnowledgeNavigator:
    def __init__(self, root):
        ...
        if SPEECH_AVAILABLE:
            ...
            self.recognizer.pause_threshold = 0.8  # 增加停顿阈值，适应更长的句子
...
"""
    ...
    #
    #     # 查找所有特殊标记
    #     bold_ranges = self.find_all_pairs(line, "**", "**")
    #     italic_ranges = self.find_all_pairs(line, "*", "*")
    #     code_ranges = self.find_all_pairs(line, "`", "`")
    #
    #     # 合并所有范围并排序
    #     all_ranges = []
    #     for start, end in bold_ranges:
    #         all_ranges.append((start, end, "bold"))
    #     for start, end in italic_ranges:
    #         all_ranges.append((start, end, "italic"))
    #     for start, end in code_ranges:
    #         all_ranges.append((start, end, "code"))
    #
    #     all_ranges.sort(key=lambda x: x[0])
    #
    #     # 应用格式
    #     for start, end, tag in all_ranges:
    #         # 插入任何未格式化的文本
    #         if start > text_pos:
    #             self.content_text.insert(tk.END, line[text_pos:start])
    #
    #         # 插入格式化文本 (去除标记符号)
    #         if tag == "bold":
    #             self.content_text.insert(tk.END, line[start + 2:end - 2], tag)
    #         elif tag == "italic":
    #             self.content_text.insert(tk.END, line[start + 1:end - 1], tag)
    #         elif tag == "code":
    #             self.content_text.insert(tk.END, line[start + 1:end - 1], tag)
    #
    #         text_pos = end
    ...
    #         self.content_text.insert(tk.END, line[text_pos:])
    #
    #     self.content_text.insert(tk.END, "\n")
    def process_inline_markdown(self, line):
        """处理行内Markdown格式"""
